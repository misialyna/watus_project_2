...
class StandaloneWatusInterface:
    def __init__(self):
        ...
        # Subscribe to existing topics - Włącz wszystkie istotne topic'y
        ...
        self.watus_subscriber.setsockopt_string(zmq.SUBSCRIBE, "tts.speak")
        self.watus_subscriber.setsockopt_string(zmq.SUBSCRIBE, "watus.state")
        self.watus_subscriber.setsockopt_string(zmq.SUBSCRIBE, "")
        # Pusta subskrypcja żeby odbierać wszystkie topic'y
    ...
    def set_emotion(self, emotion, source='auto'):
        ...
        # ALWAYS broadcast, even if same emotion - for debugging
        ...
        if emotion != old_emotion:
            print(f"🎭 STANDALONE: Emotion CHANGED from {old_emotion} to {emotion} (source: {source})")
        else:
            print(f"🎭 STANDALONE: Emotion set to {emotion} (source: {source}) - same as before")
    ...
    def analyze_dialog_message(self, topic, message):
        ...
        try:
            if topic == "dialog.leader":
                ...
                text_full = dialog_data.get('text_full', '')
                is_leader = dialog_data.get('is_leader', False)
                timestamp = dialog_data.get('timestamp', '')

                self.last_speaker = speaker

                if is_leader and speaker == 'leader':
                    self.set_emotion('happy', source='dialog_leader')
                elif speaker == 'unknown':
                    self.set_emotion('standby', source='dialog_unknown')
                else:
                    self.set_emotion('processing', source='dialog_processing')

            elif topic == "unknown_utterance":
                self.set_emotion('standby', source='dialog_unknown')
                print("❓ Unknown utterance received")

            elif topic == "watus.state":
                # Parse state message from watus.py
                state_data = json.loads(message)
                state = state_data.get('state', 'standby')

                # Map watus states to emotions
                emotion_map = {
                    "listening": "listening",
                ...
                emotion = emotion_map.get(state, "standby")
                self.set_emotion(emotion, source='watus_state')
                print(f"🚀 State update: {state} -> emotion: {emotion}")

            elif topic == "tts.speak":
                # TTS is speaking
                self.set_emotion('speaking', source='tts_speak')
        ...
        while self.running:
            try:
                ...
                if self.watus_subscriber in socks:
                    # Odbierz multipart wiadomość (ZMQ używa multipart)
                    frames = self.watus_subscriber.recv_multipart()

                    if len(frames) >= 2:
                        # Topic i dane JSON
                        topic_bytes, data_bytes = frames[0], frames[1]
                        topic = topic_bytes.decode('utf-8', errors='ignore')
                        data = data_bytes.decode('utf-8', errors='ignore')

                        self.analyze_dialog_message(topic, data)
                        print(f"📨 ZMQ: {topic} ({len(data)} chars)")
                    elif len(frames) == 1:
                        # Tylko topic bez danych
                        topic = frames[0].decode('utf-8', errors='ignore')
                        self.analyze_dialog_message(topic, '{}')
                        print(f"📨 ZMQ: {topic} (bez danych)")
                    else:
                        print(f"⚠️  Nieprawidłowa wiadomość ZMQ: {len(frames)} frame'y")
